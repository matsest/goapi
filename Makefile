...
## Dev pipeline: Generate code, format, lint and build
dev: generate format lint.fast build
...
get.tools:
	# License checker
#	go get -u github.com/frapposelli/wwhrd@v0.3.0
	go get -u github.com/golangci/golangci-lint/cmd/golangci-lint@v1.30.0
...
## Generate go code from swagger configuration
generate:
...
.PHONY: format lint.fast lint.full 

## Format code and run full linting
...
## Tidy, verify and vendor all go modules
...
swagger.doc:
	python scripts/swagger-yaml-to-html.py < pkg/swagger/swagger.yml > doc/index.html
